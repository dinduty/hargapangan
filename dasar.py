...
@st.cache_data
def load_real_data():
    """Load data from CSV file in the same directory as this script"""
    
    # Dapatkan lokasi folder tempat script ini berada
    script_dir = Path(__file__).parent.absolute()
    
    # Cari file CSV di folder tersebut
    possible_files = [
    ...
    for file_path in possible_files:
        if file_path.exists():
            try:
                ...
                # Cek kolom yang ada
                st.sidebar.write(f"📁 Kolom yang ditemukan: {list(df.columns)}")
                ...
                if 'tanggal' in df.columns:
                    ...
                    st.sidebar.error("Kolom 'tanggal' tidak ditemukan di file CSV!")
                    return None
                
                if 'harga' not in df.columns and 'price' in df.columns:
                    df['harga'] = df['price']
                
                st.sidebar.success(f"✅ Berhasil load {len(df)} baris dari: {file_path.name}")
                return df
                
            except Exception as e:
                st.sidebar.warning(f"Error baca {file_path.name}: {e}")
    
    st.sidebar.warning("⚠️ File CSV tidak ditemukan. Gunakan data dummy.")
    return None

# Panggil fungsi load data
df_real = load_real_data()

# Tampilkan preview di sidebar jika berhasil
if df_real is not None:
    with st.sidebar.expander("📊 Preview Data Real"):
        st.dataframe(df_real.head(3))
...
def sarima_section(province, commodity):
    ...
    if df_real is not None:
        df_filtered = df_real[
            (df_real['provinsi'] == province) & 
            (df_real['komoditas'] == commodity)
        ]
        if not df_filtered.empty:
            df_price = df_filtered[['tanggal', 'harga']].copy()
        else:
